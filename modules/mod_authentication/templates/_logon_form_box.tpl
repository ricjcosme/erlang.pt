<div id="logon_form_box">
	<form id="logon_form" method="post" action="postback">
		<h1 class="logon_header">{_ Autentique-se em _} <span>{{ m.config.site.title.value|default:"Zotonic" }}</span></h1>
		...
		<div id="logon_rememberme" class="rememberme">
			...
			<label for="{{ #rememberme }}">{_ Permanecer autenticado até fazer log off. _}</label>
		</div>

		<div class="clearfix"></div>

		<div id="logon_button">
			<button type="submit" class="button">{_ Autentique-se _}</button>
		</div>
	</form>
</div>
