...
-define(TRANS_PAGER_NEXT, {trans, [{en, "próximo"}, {nl, "volgende"}]}).
-define(TRANS_PAGER_PREV, {trans, [{en, "anterior"}, {nl, "vorige"}]}).
...
prev(Page, _Pages, _Dispatch, _DispatchArgs, Context) when Page =< 1 ->
    ["\n<span class=\"disabled pager-prev\">&laquo; ", z_trans:trans(?TRANS_PAGER_PREV, Context), "</span>"];
prev(Page, _Pages, Dispatch, DispatchArgs, Context) ->
    Url = z_dispatcher:url_for(Dispatch, [{page,Page-1}|DispatchArgs], Context),
    ["\n<span class=\"pager-prev\"><a href=\"",Url,"\">&laquo; ", z_trans:trans(?TRANS_PAGER_PREV, Context), "</a></span>"].

next(Page, Pages, _Dispatch, _DispatchArgs, Context) when Page >= Pages ->
    ["\n<span class=\"disabled pager-next\">", z_trans:trans(?TRANS_PAGER_NEXT, Context), " &raquo;</span>"];
next(Page, _Pages, Dispatch, DispatchArgs, Context) ->
    Url = z_dispatcher:url_for(Dispatch, [{page,Page+1}|DispatchArgs], Context),
    ["\n<span class=\"pager-next\"><a href=\"",Url,"\">", z_trans:trans(?TRANS_PAGER_NEXT, Context), " &raquo;</a></span>"].


url_to_li(sep, _, _) ->
    "\n<span class=\"pager-sep\">…</span>";
url_to_li(Url, N, false) ->
    ["\n<span class=\"pager-page\"><a href=\"",Url,"\">",integer_to_list(N),"</a></span>"];
url_to_li(Url, N, true) ->
    ["\n<span class=\"pager-page current\"><a href=\"",Url,"\">",integer_to_list(N),"</a></span>"].
